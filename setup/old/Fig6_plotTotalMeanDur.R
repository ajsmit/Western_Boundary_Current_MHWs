...
totalMeanDur <- function(events) {
  totMean <- events %>%
    dplyr::group_by(lon, lat) %>%
    dplyr::summarise(y = sum(duration))
}

AC_totalMeanDur <- totalMeanDur(AC_events)
BC_totalMeanDur <- totalMeanDur(BC_events)
EAC_totalMeanDur <- totalMeanDur(EAC_events)
KC_totalMeanDur <- totalMeanDur(KC_events)
GS_totalMeanDur <- totalMeanDur(GS_events)

# Agulhas Current
AC_fig6 <- ggplot(AC_totalMeanDur, aes(x = lon, y = lat)) +
  ...
  labs(title = "Total mean duration (1983-2012)", x = NULL, y = NULL)

ggplot2::ggsave("figures/Fig6_totalMeanDur_AC.png", width = 8.25, height = 5.5, scale = 0.7)

# Brazil Current
BC_fig6 <- ggplot(BC_totalMeanDur, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nduration\n(days)") +
...
EAC_fig6 <- ggplot(EAC_totalMeanDur, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nduration\n(days)") +
...
# Kuroshio Current
KC_fig6 <- ggplot(KC_totalMeanDur, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nduration\n(days)") +
...
# Gulf Stream Current
GS_fig6 <- ggplot(GS_totalMeanDur, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nduration\n(days)") +
...
library(ggpubr)
l <- get_legend(AC_fig6)
ggarrange(AC_fig6 + theme(legend.position = 'none'),
          BC_fig6 + theme(legend.position = 'none'),
          EAC_fig6 + theme(legend.position = 'none'),
          KC_fig6 + theme(legend.position = 'none'),
          GS_fig6 + theme(legend.position = 'none'),
          l,
          ncol = 2, nrow = 3)
ggplot2::ggsave("figures/Fig6_totalMeanDur.pdf", width = 8.25, height = 9, scale = 1.1)
