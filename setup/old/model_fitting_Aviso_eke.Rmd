...
sl.in <- as.tibble(fread("/Volumes/Benguela/spatial/processed/Aviso+/WBC/subregions/all-sub-summarised-Aviso+-19930101-20181023.csv"))
sl <- sl.in %>%
...
sl.d <- as.tibble(ddply(sl, .(region, location), 
                        function(df) { make_whole(df, x = t, y = eke) },
                        .parallel = TRUE)) %>%
...
detect.ev <- function(sl) {
  out <- RmarineHeatWaves::detect(sl, x = t, y = eke, min_duration = 5, 
...
daily.dat <- clm %>%
  mutate(de.seas = eke - seas_clim_year) %>%
  group_by(region, location) %>%
  mutate(anom = eke - mean(eke, na.rm = TRUE),
         num = seq(1:length(eke))) %>%
  nest()
```
...
monthly.dat <- daily.dat %>%
  ...
  dplyr::group_by(region, location) %>%
  dplyr::mutate(de.seas2 = eke - seas_clim_year,
                nMonth = month(t)) %>% 
  dplyr::mutate(num = seq(1:length(eke))) %>%
  na.omit() %>% 
  nest()
```
...
theme_set(grey_update2)
gam.1 %>%
  ggplot(aes(x = t, y = eke, group = location)) +
  ...
  facet_grid(region ~ .) +
  scale_x_date(limits = as.Date(c("1981-09-01", "2018-09-30"))) +
  scale_colour_manual(values = c("red3", "blue3", "green4")) +
