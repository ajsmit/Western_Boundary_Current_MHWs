...
lats <- c(-90, 90)
lons <- c(0, 360)
shore <- importGSHHS(paste0(gshhsDir, "/gshhs_i.b"),
                     xlim = lons, ylim = lats,
                     maxLevel = 1, useWest = FALSE)
...
totalMeanInt <- function(events) {
  totMean <- events %>%
    dplyr::group_by(lon, lat) %>%
    dplyr::summarise(y = sum(int_mean))
}

AC_totalMeanInt <- totalMeanInt(AC_events)
BC_totalMeanInt <- totalMeanInt(BC_events)
EAC_totalMeanInt <- totalMeanInt(EAC_events)
KC_totalMeanInt <- totalMeanInt(KC_events)
GS_totalMeanInt <- totalMeanInt(GS_events)

# Agulhas Current
AC_fig5 <- ggplot(AC_totalMeanInt, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nintensity\n(°C)") +
  geom_vline(xintercept = seq(10, 45, 5),
             linetype = "dashed", size = 0.2) +
  geom_hline(yintercept = seq(-45, -20, 5),
             linetype = "dashed", size = 0.2) +
  ...
  labs(title = "Total mean intensity (1983-2012)", x = NULL, y = NULL)

ggplot2::ggsave("figures/Fig5_totalMeanInt_AC.png", width = 8.25, height = 5.5, scale = 0.7)

# Brazil Current
BC_fig5 <- ggplot(BC_totalMeanInt, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nintensity\n(°C)") +
...
EAC_fig5 <- ggplot(EAC_totalMeanInt, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nintensity\n(°C)") +
...
# Kuroshio Current
KC_fig5 <- ggplot(KC_totalMeanInt, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nintensity\n(°C)") +
...
# Gulf Stream Current
GS_fig5 <- ggplot(GS_totalMeanInt, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = y), interpolate = FALSE) +
  scale_fill_viridis(name = "Total:\nmean\nintensity\n(°C)") +
...
library(ggpubr)
l <- get_legend(AC_fig5)
ggarrange(AC_fig5 + theme(legend.position = 'none'),
          BC_fig5 + theme(legend.position = 'none'),
          EAC_fig5 + theme(legend.position = 'none'),
          KC_fig5 + theme(legend.position = 'none'),
          GS_fig5 + theme(legend.position = 'none'),
          l,
          ncol = 2, nrow = 3)
ggplot2::ggsave("figures/Fig5_totalMeanInt.pdf", width = 8.25, height = 9, scale = 1.1)
