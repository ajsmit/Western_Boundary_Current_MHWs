...
eventCumIntFun <- function(events) {
  freq <- events %>%
    ...
    dplyr::summarise(y = mean(int_cum))
}
...
# AC_annualCumIntTrend$slope <- round(AC_annualCumIntTrend$slope, 1)
AC_fig9 <- ggplot(AC_annualCumIntTrend, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = slope), interpolate = FALSE) +
  scale_fill_gradient2(name = "Trend:\ncumulative\nintensity\n(°C.days/dec)", low = "#243c80", mid = "gray98",
                       high = "#b6272f", midpoint = 0) + #, limits = c(-2.5, 9.5)
  ...
  labs(title = "Trend: cumulative intensity/dec (1983-2012)", x = NULL, y = NULL)

ggplot2::ggsave("figures/Fig9_trendCumInt_AC.png", width = 8.25, height = 5.5, scale = 0.7)

BC_annualCumIntTrend$slope <- round(BC_annualCumIntTrend$slope, 1)
BC_fig9 <- ggplot(BC_annualCumIntTrend, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = slope), interpolate = FALSE) +
  scale_fill_gradient2(name = "Trend:\ncumulative\nintensity\n(°C.days/dec)", low = "blue", mid = "gray98",
                       high = "red3", midpoint = 0, limits = c(-20.5, 270.5)) + #, limits = c(-2.5, 20)
...
EAC_annualCumIntTrend$slope <- round(annualCumIntTrend$slope, 1)
EAC_fig9 <- ggplot(EAC_annualCumIntTrend, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = slope), interpolate = FALSE) +
  scale_fill_gradient2(name = "Trend:\ncumulative\nintensity\n(°C.days/dec)", low = "blue", mid = "gray98",
                       high = "red3", midpoint = 0, limits = c(-20.5, 270.5)) + #, limits = c(-2.5, 20)
  # geom_point(aes(x = lon, y = lat, colour = pbracket), shape = "/", size = 0.25) +
...
KC_annualCumIntTrend$slope <- round(annualCumIntTrend$slope, 1)
KC_fig9 <- ggplot(KC_annualCumIntTrend, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = slope), interpolate = FALSE) +
  scale_fill_gradient2(name = "Trend:\ncumulative\nintensity\n(°C.days/dec)", low = "blue", mid = "gray98",
                       high = "red3", midpoint = 0, limits = c(-20.5, 270.5)) + #, limits = c(-2.5, 20)
  # geom_point(aes(x = lon, y = lat, colour = pbracket), shape = "/", size = 0.25) +
...
GS_annualCumIntTrend$slope <- round(annualCumIntTrend$slope, 1)
GS_fig9 <- ggplot(GS_annualCumIntTrend, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = slope), interpolate = FALSE) +
  scale_fill_gradient2(name = "Trend:\ncumulative\nintensity\n(°C.days/dec)", low = "blue", mid = "gray98",
                       high = "red3", midpoint = 0, limits = c(-20.5, 270.5)) + #, limits = c(-2.5, 27.5)
  # geom_point(aes(x = lon, y = lat, colour = pbracket), shape = "/", size = 0.25) +
  geom_contour(aes(x = lon, y = lat, z = pval), binwidth = 0.05, breaks=c(0.05), colour = "red4", size = 0.2) +
  # scale_colour_manual(breaks = c("(0,0.001]", "(0.001,0.01]", "(0.01,0.05]", "(0.05,1]"),
  #                     values = c("black", "black", "black", NA),
...
library(ggpubr)
l <- get_legend(AC_fig9)
ggarrange(AC_fig9 + theme(legend.position = 'none'),
          BC_fig9 + theme(legend.position = 'none'),
          EAC_fig9 + theme(legend.position = 'none'),
          KC_fig9 + theme(legend.position = 'none'),
          GS_fig9 + theme(legend.position = 'none'),
          l,
          ncol = 2, nrow = 3)
ggplot2::ggsave("figures/Fig9_trendCumInt.pdf", width = 8.25, height = 9, scale = 1.1)
