---
title: "Model fitting (MW_OI_REMSS)"
author:
...
source("/Volumes/GoogleDrive/My Drive/SACTN/Ocean_MHW/Changing_nature_of_ocean_heat/functions/models.R")
```
...
sst.in <- as.tibble(fread("/Volumes/Benguela/spatial/processed/MW_OI-REMSS-L4-GLOB-v4.0/WBC/daily/subregions/AC-sub-summarised-MW_OI-REMSS-L4-GLOB-v4.0-19980101-20171231.csv"))
sst <- sst.in %>%
  mutate(t = fastDate(t)) %>%
...
sst.d <- as.tibble(ddply(sst, .(region, location), 
...
detect.ev <- function(sst) {
  out <- RmarineHeatWaves::detect(sst, x = t, y = temp, min_duration = 5, 
                                  ...
                                  climatology_start = "1998-01-01", 
                                  climatology_end = "2017-12-31")
  x <- out$event
  return(x)
  }
...
fitted.AR1 <- as.tibble(melt(mapply(function(x) gamms.AR1$mod.gamm[[x]]$fitted.values, 1:2))[3])
...
residuals.AR1 <- as.tibble(melt(mapply(function(x) 
  gls.AR1$mod.gls[[x]]$residuals, 1:2))[3])
...
source("/Volumes/GoogleDrive/My Drive/repos/miscellaneous/Gavin_Simpson/derivFun.R")
