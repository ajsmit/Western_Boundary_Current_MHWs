---
  title: "Initial analysis: Brazil Current"
author:
...
```{r flame-graphs}
plt1 <- flame.plot(climatology, "BC", "reference")
plt2 <- flame.plot(climatology, "BC", "current")
plt3 <- flame.plot(climatology, "BC", "event")

ggarrange(plt1,
          plt2,
          plt3,
          ncol = 1, nrow = 3)
# ggplot2::ggsave("../Brazil_Current/BC_OISST_flame_2010-2018.pdf",
#                 width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)
```

```{r load-current-eke}
vel_BC <- as.tibble(fread(paste0(AvisoDir, "/BC-dataset-duacs-rep-global-merged-allsat-phy-l4-v3.csv"),
...
```{r current-eke-map}
vel.plot(BC_ev1, bathy_BC, BC.layers) + labs(subtitle = paste(top.ev[1, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_peak_mean_intensity_map_", top.ev[1, 2], ".pdf"),
#                 width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)

vel.plot(BC_ev2, bathy_BC, BC.layers) + labs(subtitle = paste(top.ev[2, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_peak_mean_intensity_map_", top.ev[2, 2], ".pdf"),
#                 width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)

vel.plot(BC_ev3, bathy_BC, BC.layers) + labs(subtitle = paste(top.ev[3, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_peak_mean_intensity_map_", top.ev[3, 2], ".pdf"),
                # width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)
```

```{r load-daily-OISST}
# load the temp and climatology
sst <- fread(paste0(OISSTDir, "/BC-avhrr-only-v2.19810901-20180930_climatology.csv"),
             colClasses = list("numeric" = 1:3, "myDate" = 4, "numeric" = 5:7))

# load the events
ev <- as.tibble(fread(paste0(evDir, "/BC-avhrr-only-v2.19810901-20180930_events.csv")))
```

```{r Hovmöller-prep}
hov1 <- sst %>%
  dplyr::filter(lon >= 305 & lon <= 315) %>%
...
hov.plt <- function(data, start, peak, end) {
  ...
  scale_fill_gradientn(name = "SST anomaly (°C)",
                       colours = col1, limits = c(-10.5, 10.5), position = "bottom") +
  ...
  geom_vline(aes(xintercept = 306), alpha = 0.8, linetype = "dotted") +
  geom_vline(aes(xintercept = 307), alpha = 0.8, linetype = "dotted") +
...
# ggplot2::ggsave(paste0("../Brazil_Current/BC_Hovmöller_", top.ev[1, 2], ".png"),
...
# ggplot2::ggsave(paste0("../Brazil_Current/BC_Hovmöller_", top.ev[2, 2], ".png"),
#                 width = 5, height = 3.5, units = "cm", dpi = "retina", scale = 2.8)

# 2018-03-05 2018-04-05 2018-05-02
hov.plt(hov3, top.ev[3, 1], top.ev[3, 2], top.ev[3, 3]) + labs(subtitle = paste(top.ev[3, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_Hovmöller_", top.ev[3, 2], ".png"),
#                 width = 5, height = 3.5, units = "cm", dpi = "retina", scale = 2.8)
```
...
```{r sst-anomaly-plot}
sst.plot(sst.ev1, bathy_BC, BC.layers) + labs(subtitle = paste(top.ev[1, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_SST_anom_map_", top.ev[1, 2], ".pdf"),
#                 width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)

sst.plot(sst.ev2, bathy_BC, BC.layers) + labs(subtitle = paste(top.ev[2, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_SST_anom_map_", top.ev[2, 2], ".pdf"),
#                 width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)

sst.plot(sst.ev3, bathy_BC, BC.layers) + labs(subtitle = paste(top.ev[3, 2]))
# ggplot2::ggsave(paste0("../Brazil_Current/BC_SST_anom_map_", top.ev[3, 2], ".pdf"),
#                 width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)
```
