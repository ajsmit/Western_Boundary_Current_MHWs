...
source("setup/regional_analyses/1.regional_setup.R")
...
sst.plot(sst.ev2, bathy_AC, AC.layers) + labs(subtitle = paste(top.ev[2,2]))
ggplot2::ggsave(paste0("setup/regional_analyses/Agulhas_Current/AC_SST_anom_map_", top.ev[2,2], ".png"),
                width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)

sst.plot(sst.ev4, bathy_AC, AC.layers) + labs(subtitle = paste(top.ev[4,2]))
ggplot2::ggsave(paste0("setup/regional_analyses/Agulhas_Current/AC_SST_anom_map_", top.ev[4,2], ".png"),
                width = 7.0 * (1/3), height = 5 * (1/3), scale = 3.5)

sst.plot(sst.ev5, bathy_AC, AC.layers) + labs(subtitle = paste(top.ev[5,2]))
