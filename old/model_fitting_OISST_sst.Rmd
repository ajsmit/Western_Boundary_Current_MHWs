...
knitr::opts_chunk$set(include = FALSE, echo = FALSE, warning = FALSE,
                      ...
                      fig.align = 'left', fig.path = 'figures/OISST-')
```
...
sst.d1 <- sst %>%
  ...
  nest() %>% 
  dplyr::mutate(data_whole = map(data, mw))


# sst.d2 <- as.tibble(ddply(sst, .(region, location), 
#                         function(df) { make_whole(df, x = t, y = temp) },
#                         .parallel = TRUE)) %>%
...
detect.ev <- function(sst) {
  out <- RmarineHeatWaves::detect(sst, x = t, y = temp, min_duration = 5, 
...
ev.raw <- sst.d1 %>%
  dplyr::mutate(ev.raw = map(data_whole, detect.ev)) %>%
  unnest(ev.raw)

clm <- sst.d1 %>%
  dplyr::mutate(clm = map(data_whole, make.clm)) %>%
  unnest(clm)
```
...
monthly.dat <- daily.dat %>%
  ...
  dplyr::group_by(region, location) %>%
  dplyr::mutate(de.seas2 = temp - seas_clim_year,
                nMonth = month(t)) %>% 
...
gam.1 %>%
  ggplot(aes(x = t, y = temp, group = location)) +
...
gam.1 %>%
  ...
  ggplot(aes(x = t, y = temp, group = location)) +
  geom_hline(aes(yintercept = 0), size = 0.5, colour = "grey50") +
  geom_line(aes(colour = location), size = 0.1, alpha = 0.8) +
  geom_line(aes(y = .fitted, colour = location), alpha = 1.0, size = 0.75) +
  scale_x_date(limits = as.Date(c("1981-09-01", "2017-10-19"))) +
  scale_colour_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero-centered (°C)",
       title = "GAM -- daily raw data")

ggplot2::ggsave("../figures/Fig13_dailyGam_AC.png", width = 8.25, height = 2.5, scale = 1.1)
```
...
gam.3 %>%
  ...
  labs(x = "Date", y = "Temperature, zero-centered (°C)",
       title = "GAM -- daily de-seasoned data")
ggplot2::ggsave("../figures/Fig14_dailyGam_sansSeason_AC.png", width = 8.25, height = 2.5, scale = 1.1)
```
...
gam.3 %>%
  ...
  labs(x = "Date", y = "Temperature, zero-centered (°C)",
       title = "GAM -- daily de-seasoned and detrended data")
ggplot2::ggsave("../figures/Fig15_dailyGam_sansSeasonTrend_AC.png", width = 8.25, height = 2.5, scale = 1.1)
```
...
fitted.AR1 <- as.tibble(melt(mapply(function(x) gamms.AR1$mod.gamm[[x]]$fitted.values, 1:10))[3])
gamms.AR1.df <- cbind(unnest(monthly.dat, data), fitted.AR1)
...
residuals.AR1 <- as.tibble(melt(mapply(function(x) 
  gls.AR1$mod.gls[[x]]$residuals, 1:10))[3])
gls.AR1.df <- cbind(unnest(monthly.dat, data), residuals.AR1)
gam.df2 <- gls.AR1.df %>%
...
plt.1a <- gam.df2 %>%
  ...
  scale_colour_manual(values = c("red4", "blue3")) +
  scale_fill_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero centered (°C)") + guides(fill = FALSE)
...
plt.1a_AC <- gam.df2 %>%
  ...
  # geom_vline(data = ev, aes(xintercept = date_start, colour = location), alpha = 0.1) +
  geom_hline(aes(yintercept = 0), size = 0.2, colour = "grey50") +
  geom_line(aes(colour = location), size = 0.25, alpha = 1) +
  geom_rug(data = filter(ev.raw, location == "in"),
           aes(x = date_start, y = int_max_abs), col = "red4", sides = "b") +
  geom_rug(data = filter(ev.raw, location == "out"),
           aes(x = date_start, y = int_max_abs), col = "blue3", sides = "t") +
  # geom_line(aes(y = de.seas), colour = "black", size = 0.2, alpha = 0.6) +
  geom_ribbon(aes(ymin = (.fitted - .se.fit), ymax = (.fitted + .se.fit), 
                  fill = location), alpha = 0.7) +
  # geom_line(aes(y = .fitted), colout = "black", alpha = 0.7, size = 0.55) +
  ...
  labs(x = "Date", y = "Temp. (°C)",
       title = "GAM -- monthly") + guides(fill = FALSE)
...
plt.1b_AC <- ev.1 %>% 
  ...
  labs(x = "Date", y = "Events/yr",
       title = "GLS, Poissan link") + theme(legend.position = "right")
...
ggarrange(plt.1a_AC + theme(legend.position = 'none'),
          plt.1b_AC, 
          plt.2a_AC + theme(legend.position = 'none'),
          plt.2b_AC,
          ncol = 2, nrow = 2,
          widths = c(3/5, 2/5), align = 'hv')

ggplot2::ggsave("../figures/Fig16_dailyGam_events_AC.png", width = 8.25, height = 4.5, scale = 1.1)
```
...
AC <- ggplot(data = AC.pred$predicted, aes(x = t, group = location)) +
  ...
  geom_line(aes(y = incr), colour = "black", size = 0.8) +
  geom_line(aes(y = decr), colour = "black", size = 0.8) +
  facet_grid(location ~ .) +
  scale_x_date(limits = as.Date(c("1981-09-01", "2017-10-19"))) +
  scale_color_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero centered (°C)") + ggtitle("Agulhas Current") 

ggplot2::ggsave("FigX_trendGAM_AC.png", width = 8.25, height = 5.5, scale = 0.7)
...
BC <- ggplot(data = BC.pred$predicted, aes(x = t, group = location)) +
  ...
  facet_grid(location ~ .) +
  scale_color_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero centered (°C)") + ggtitle("Brazil Current")
...
EAC <- ggplot(data = EAC.pred$predicted, aes(x = t, group = location)) +
  ...
  facet_grid(location ~ .) +
  scale_color_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero centered (°C)") + ggtitle("East Australian Current")
...
KC <- ggplot(data = KC.pred$predicted, aes(x = t, group = location)) +
  ...
  facet_grid(location ~ .) +
  scale_color_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero centered (°C)") + ggtitle("Kuroshio Current")
...
GS <- ggplot(data = GS.pred$predicted, aes(x = t, group = location)) +
  ...
  facet_grid(location ~ .) +
  scale_color_manual(values = c("red4", "blue3")) +
  labs(x = "Date", y = "Temperature, zero centered (°C)") + ggtitle("Gulf Stream")
