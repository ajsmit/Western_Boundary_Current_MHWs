...
clim <- clim[lon >= 140 & lon <= 150, ] # trim to a section of lon that falls across area of maximal MHW activity
clim <- clim[lat >= 32.5 & lat <= 42.5, ]
...
ex2 <- ex[, list(ex = mean(temp - thresh)), by = list(lat = lat, t = t)]
rm(clim)
...
ke <- ke[lon >= 140 & lon <= 150, ]
ke <- ke[lat >= 32.5 & lat <= 42.5, ]
...
ke2 <- ke[, list(eke = mean(eke),
                 ...
                 eke_roll = mean(eke_roll)), by = list(lat = lat, t = t)]
...
save(dat.r.long, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/KC_correlations_v4.RData")

# Make a graph
KC.corr.plt <- ggplot(data = dat.r.long, aes(x = lat, group = ke.type)) +
...
save(KC.corr.plt, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/KC_corr_plt_v1.RData")

ggplot2::ggsave("publ_plots/Fig0x_KC_zonal.jpg",
