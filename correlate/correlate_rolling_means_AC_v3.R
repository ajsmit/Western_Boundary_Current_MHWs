...
clim <- clim[lon >= 10 & lon <= 25, ] # trim to a section of lon that falls across area of maximal MHW activity
...
rm(clim) # prevents "Error: vector memory exhausted (limit reached?)"
...
save(dat.r.long, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/AC_correlations_v4.RData")

# Make a graph
AC.corr.plt <- ggplot(data = dat.r.long, aes(x = lat, group = ke.type)) +
...
save(AC.corr.plt, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/AC_corr_plt_v1.RData")

ggplot2::ggsave("publ_plots/Fig0x_AC_zonal.jpg",
