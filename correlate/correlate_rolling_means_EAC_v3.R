...
clim <- clim[lon >= 150 & lon <= 157.5, ] # trim to a section of lon that falls across area of maximal MHW activity
clim <- clim[lat >= -40 & lat <= -32.5, ]
...
# ex2[, c("doy", "temp", "seas", "thresh") := NULL]
rm(clim)
...
ke <- ke[lon >= 150 & lon <= 157.5, ]
ke <- ke[lat >= -40 & lat <= -32.5, ]
...
save(dat.r.long, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/EAC_correlations_v4.RData")

# Make a graph
EAC.corr.plt <- ggplot(data = dat.r.long, aes(x = lon, group = ke.type)) +
...
save(EAC.corr.plt, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/EAC_corr_plt_v1.RData")

ggplot2::ggsave("publ_plots/Fig0x_EAC_meridional.jpg",
