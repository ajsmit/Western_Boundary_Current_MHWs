...
clim <- clim[lon >= 290 & lon <= 300, ] # trim to a section of lon that falls across area of maximal MHW activity
clim <- clim[lat >= 35 & lat <= 45, ]
...
save(dat.r.long, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/GS_correlations_v4.RData")

# Make a graph
GS.corr.plt <- ggplot(data = dat.r.long, aes(x = lat, group = ke.type)) +
...
save(GS.corr.plt, file = "/Volumes/Benguela/spatial/processed/WBC/misc_results/GS_corr_plt_v1.RData")

ggplot2::ggsave("publ_plots/Fig0x_GS_zonal.jpg",
